...
def article_overview(kind, period):
    ...
    # YOUR CODE HERE
    '''
    #get most viewd article
    viewlist = []
    for elem in data:
        viewlist.append([elem["id"],elem["views"]])
    print viewlist
    #topart = [e[0] for e in viewlist if e[1] == max(viewlist, key=lambda x: x[1])]
    topart = sorted(viewlist, key = lambda x:x[1],reverse=True)[0][0]
    print topart
    '''
    labels = []
    for elem in data:
        labels.append({elem["section"].encode("utf8"):elem["title"].encode("utf8")})
    #print labels

    medias = []
    for elem in data:
        for el in elem["media"]:
            for e in el["media-metadata"]:
                #print e[u'url']
                if e[u'format'] == "Standard Thumbnail":
                    medias.append(e[u'url'])
    #print medias
    titles = labels
    urls = medias
    return (titles, urls)
