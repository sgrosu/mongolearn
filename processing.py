...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            temp_dict = {}
            temp_dict['classification'] = {}
            for elem in process_fields:
              if elem == 'rdf-schema#label':
                try:
                  a = re.findall('(\(\S+\))',line[elem])[0]
                  #print a
                  
                  temp_dict[fields[elem]] = line[elem][:line[elem].find(a)].strip()
                except:
                  temp_dict[fields[elem]] = line[elem].strip()
              elif elem == 'name':
                try:
                  re.match('[^([a-zA-Z0-9\s)]',line[elem])
                  temp_dict[fields[elem]] = temp_dict['label'].strip()
                except:
                  if line[elem] == 'NULL' or line[elem] == '':
                    temp_dict[fields[elem]] = temp_dict['label'].strip()
                  else:
                    temp_dict[fields[elem]] = line['name'].strip()
              elif elem == 'synonym':
                
                if line[elem] != 'NULL':
                  if line[elem].startswith('{'):
                    s_arr = []
                    syn_array = line[elem].strip('{').strip('}').split('|')
                    for el in syn_array:
                      s_arr.append(el.strip('*').strip())

                    temp_dict[fields[elem]] = s_arr
                  else:
                    temp_dict[fields[elem]] = [line[elem].strip()]
                else:
                  temp_dict[fields[elem]] = None
              elif elem in ['kingdom_label','family_label','order_label','phylum_label','genus_label','class_label']:
                if line[elem] == 'NULL' or line[elem] == '':
                  temp_dict['classification'][fields[elem]] = None
                else:
                  temp_dict['classification'][fields[elem]] = line[elem].strip()
              else:
                temp_dict[fields[elem]] = line[elem].strip()
...
'''
print process_file(DATAFILE,FIELDS)
'''